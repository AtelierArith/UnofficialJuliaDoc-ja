...
ルートタスクと、新しく生成されたタスクが10のさらに新しいタスクの親であることがわかります。
...
プローブ宣言は `usdt` の種類を取り、その後にライブラリへのパスまたはPID、プロバイダー名 `julia`、プローブ名 `gc__begin` を指定します。`libjulia-internal.so` への相対パスを使用していますが、これは本番システムでは絶対パスである必要があるかもしれません。
