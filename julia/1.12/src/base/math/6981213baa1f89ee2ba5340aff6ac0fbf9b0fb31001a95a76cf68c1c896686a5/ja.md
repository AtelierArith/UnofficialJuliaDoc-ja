...
いくつかのUnicode文字を使用して、新しい二項演算子を定義し、インフィックス表記をサポートすることができます。たとえば、`⊗(x,y) = kron(x,y)`は、`⊗`（テンソル積）関数をクロネッカー積として定義し、インフィックス構文を使用して二項演算子として呼び出すことができます：`C = A ⊗ B`、および通常のプレフィックス構文`C = ⊗(A,B)`でも呼び出すことができます。
