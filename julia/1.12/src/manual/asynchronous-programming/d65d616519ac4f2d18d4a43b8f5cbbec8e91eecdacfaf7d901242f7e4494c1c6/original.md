...
It is common to want to create a task and schedule it right away, so the macro [`Threads.@spawn`](@ref) is provided for that purpose –- `Threads.@spawn x` is equivalent to `task = @task x; task.sticky = false; schedule(task)`.
