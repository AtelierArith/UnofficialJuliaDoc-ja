...
LibGit2モジュールは、[libgit2](https://libgit2.org/)へのバインディングを提供します。これは、[Git](https://git-scm.com/)バージョン管理システムのコア機能を実装したポータブルCライブラリです。これらのバインディングは現在、Juliaのパッケージマネージャを支えるために使用されています。このモジュールは最終的に別のパッケージに移動されることが期待されています。

### Functionality

このドキュメントの一部は、libgit2 APIに関する事前知識を前提としています。ここで参照されているいくつかのオブジェクトやメソッドに関する詳細については、上流の [libgit2 API reference](https://libgit2.org/libgit2/#v1.0.0) を参照してください。
