`BroadcastStyle`は抽象型であり、ブロードキャスト時のオブジェクトの動作を決定するために使用されるトレイト関数です。`BroadcastStyle(typeof(x))`は`x`に関連付けられたスタイルを返します。型のブロードキャスト動作をカスタマイズするには、型/メソッドペアを定義することでスタイルを宣言できます。
