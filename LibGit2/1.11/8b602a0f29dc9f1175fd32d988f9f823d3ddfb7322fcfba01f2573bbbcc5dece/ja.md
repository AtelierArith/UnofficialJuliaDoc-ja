...
!!! note
    プッシュ refspecs に関する情報を追加する方法は他に2つあります：リポジトリの `GitConfig` でオプションを設定する（キーとして `push.default` を使用）か、[`add_push!`](@ref) を呼び出すことです。それ以外の場合、`push` の呼び出しで明示的にプッシュ refspec を指定する必要があります。例えば、次のようにします：`LibGit2.push(repo, refspecs=["refs/heads/master"])`。
